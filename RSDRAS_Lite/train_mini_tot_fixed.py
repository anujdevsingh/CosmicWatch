...
class SpaceDebrisDataset(Dataset):
    ...
    def load_data_from_db(self, db_path: str) -> Tuple[List[np.ndarray], List[int]]:
        """Load data from SQLite database using correct column names"""
        try:
            conn = sqlite3.connect(db_path)
            cursor = conn.cursor()
            
            # Query space debris data using correct column names from schema
            query = """
            SELECT id, altitude, latitude, longitude, x, y, z, 
                   size, velocity, inclination, risk_score
            FROM space_debris
            WHERE altitude IS NOT NULL AND velocity IS NOT NULL
            LIMIT 5000
            """
            ...
            if len(rows) == 0:
                print("No data found in database, creating dummy data for testing")
            ...
            for row in rows:
                try:
                    id_val, altitude, latitude, longitude, x, y, z, size, velocity, inclination, risk_score = row
                    
                    # Convert the database data to TLE-like format for our enhanced features
                    # We need to estimate orbital parameters from the available data
                    tle_data = self.convert_to_tle_format(
                        altitude, latitude, longitude, x, y, z, 
                        size, velocity, inclination, risk_score
                    )
                    ...
                    # Create risk label based on existing risk_score
                    risk_label = self.calculate_risk_label_from_score(risk_score)
            ...
            if len(data) == 0:
                print("No valid data processed, creating dummy data")
                return self.create_dummy_data()
    ...
    def convert_to_tle_format(self, altitude, latitude, longitude, x, y, z, 
                             size, velocity, inclination, risk_score):
        """Convert database format to TLE-like format for enhanced features"""
        
        # Estimate orbital parameters from available data
        # Semi-major axis from altitude
        earth_radius = 6371.0  # km
        semi_major_axis = altitude + earth_radius
        
        # Mean motion from semi-major axis (Kepler's third law)
        mu = 398600.4418  # Earth's gravitational parameter
        mean_motion_rad_per_sec = np.sqrt(mu / semi_major_axis**3)
        mean_motion_rev_per_day = mean_motion_rad_per_sec * 86400 / (2 * np.pi)
        
        # Estimate eccentricity (assume mostly circular orbits)
        eccentricity = max(0.0, min(0.1, (altitude - 400) / 1000))  # Rough estimate
        
        # Use provided inclination or estimate from latitude
        if inclination is not None:
            inclination_deg = inclination
        else:
            inclination_deg = abs(latitude) if latitude is not None else 51.6
        
        # Estimate other orbital elements
        ra_of_asc_node = longitude if longitude is not None else 0.0
        arg_of_pericenter = 0.0  # Assume 0 for simplification
        mean_anomaly = np.random.uniform(0, 360)  # Random position in orbit
        
        # Use size as radar cross section
        rcs_size = size if size is not None else 1.0
        
        return {
            'MEAN_MOTION': float(mean_motion_rev_per_day),
            'ECCENTRICITY': float(eccentricity),
            'INCLINATION': float(inclination_deg),
            'RA_OF_ASC_NODE': float(ra_of_asc_node),
            'ARG_OF_PERICENTER': float(arg_of_pericenter),
            'MEAN_ANOMALY': float(mean_anomaly),
            'RCS_SIZE': float(rcs_size)
        }
    
    def calculate_risk_label_from_score(self, risk_score: float) -> int:
        """Convert risk score to categorical label"""
        if risk_score is None:
            return 0  # Default to low risk
        
        # Convert risk score (0-1) to 4 categories
        if risk_score < 0.25:
            return 0  # Low Risk
        elif risk_score < 0.5:
...
class MiniTOTTrainer:
    ...
    def train_epoch(self, train_loader, optimizer, criterion, epoch):
        ...
        for batch_idx, (sequences, labels) in enumerate(train_loader):
            ...
            if batch_idx % 20 == 0:  # More frequent updates
...
def main():
    ...
    print("🚀 Starting RSDRAS-Lite Mini-TOT Training (Fixed Version)")
    print("=" * 70)
    ...
    # Create dataset
    print("\nLoading dataset with correct database schema...")
    ...
    # Save results
    results_summary = {
        'model_type': 'Mini Temporal-Orbital Transformer (Fixed)',
    ...
    # Performance summary
    print("\n📊 RSDRAS-Lite Performance Summary:")
    print(f"   🎯 Best Validation Accuracy: {trainer.best_val_acc:.2f}%")
    print(f"   🎯 Test Accuracy: {results['accuracy']*100:.2f}%")
    print(f"   ⚡ Speed: {results['predictions_per_second']:.1f} predictions/second")
