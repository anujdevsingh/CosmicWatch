        ...
        try:
            ...
            for item in data:
                try:
                    ...
                    # Handle potential None values for period, apogee, perigee
                    period_val = item.get('PERIOD')
                    period = float(period_val) if period_val is not None else 90.0
                    
                    apogee_val = item.get('APOGEE')
                    apogee = float(apogee_val) if apogee_val is not None else 500.0
                    
                    perigee_val = item.get('PERIGEE')
                    perigee = float(perigee_val) if perigee_val is not None else 500.0
...
class SpaceDebrisDatabase:
    ...
    def __init__(self, db_path="space_debris_test.db"):
    ...
    def export_to_csv(self, filename="space_debris_export.csv"):
    ...
    def export_to_json(self, filename="space_debris_export.json"):
